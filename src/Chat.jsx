...
const Chat = () => {
  ...
  function connectToWs() {
    const ws = new WebSocket(`wss://${import.meta.env.VITE_REACT_APP_WS_API}`);
    setWs(ws);
  ...
  function sendMessage(ev, file = null) {
    ...
    if (file) {
      axios.get(`/api/v1/auth/messages/` + selectedUserId).then((res) => {
  ...
  useEffect(() => {
    axios.get(`/api/v1/auth/people`).then((res) => {
      ...
      <div className='flex flex-col bg-blue-50 w-2/3 p-2'>
        ...
        {!!selectedUserId && (
          <form className='flex gap-2 ' onSubmit={sendMessage}>
            ...
            />
            <label className='bg-blue-200 p-2 cursor-pointer text-gray-600 rounded-sm border border-blue-200'>
